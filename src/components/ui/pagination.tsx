...
const PaginationLink = React.forwardRef<HTMLAnchorElement, PaginationLinkProps>(
  ({ className, isActive, size = "icon", ...props }, ref) => (
    <a ref={ref} aria-current={isActive ? "page" : undefined} className={cn(buttonVariants({ variant: isActive ? "default" : "outline", size }), className)} {...props} />
  )
)
PaginationLink.displayName = "PaginationLink"

const PaginationPrevious = (props: React.ComponentProps<typeof PaginationLink>) => (
  <PaginationLink aria-label="Go to previous page" size="default" {...props}><ChevronLeft className="h-4 w-4" /></PaginationLink>
)

const PaginationNext = (props: React.ComponentProps<typeof PaginationLink>) => (
  <PaginationLink aria-label="Go to next page" size="default" {...props}><ChevronRight className="h-4 w-4" /></PaginationLink>
)
