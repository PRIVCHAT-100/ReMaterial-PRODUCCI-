 ...
 * Now refreshes after saving name/avatar and shows success.
 */
...
export default function AccountSection() {
  ...
  async function onSaveBasics() {
    ...
    try {
      ...
      setBusy(b => ({ ...b, saveBasics: false }))
    }
  }

  async function onChangeEmail() {
    if (!newEmail) return
    setBusy(b => ({ ...b, changeEmail: true }))
    setMessage(null)
    try {
  ...
  return (
    <div style={{ display: 'grid', gap: 24, maxWidth: 760 }}>
      ...
      {message ? (
        <div style={{ padding: 12, borderRadius: 8, background: '#F3F4F6' }}>{message}</div>
      ) : null}
    </div>
  )
}
