 ...
 * Handles missing client gracefully (no crash), same UI.
 */
...
export default function SecuritySection() {
  ...
  const isActive = useMemo(() => {
    return factors.some(f => f.status === 'verified' || f.status === 'active')
  }, [factors])
  ...
  async function onEnable() {
    ...
    try {
      ...
      if (!res.ok || !res.data) {
        setMsg(res.message ?? 'No se pudo iniciar el alta de 2FA.')
        return
      }
  ...
  async function onDisable() {
    ...
    try {
      ...
      if (!res.ok) {
        setMsg(res.message ?? 'No se pudo desactivar el 2FA.')
        return
      }
    ...
    <div style={{ display: 'grid', gap: 24, maxWidth: 760 }}>
      ...
      <section style={{ border: '1px solid #E5E7EB', borderRadius: 12, padding: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 14, color: '#6B7280' }}>
              Estado: {isActive ? <b>Activado</b> : <b>Desactivado</b>}
            </div>
          </div>
          {!isActive ? (
        ...
        {totp.step === 'enrolled' ? (
          <div style={{ display: 'grid', gap: 12 }}>
            <div style={{ fontSize: 14 }}>
              Escanea el QR con tu app de autenticación (Google Authenticator, Authy…) y escribe el código de 6 dígitos.
            </div>
            <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
              <div style={{ width: 180, height: 180, border: '1px solid #E5E7EB', borderRadius: 8, display: 'grid', placeItems: 'center', overflow: 'hidden' }}>
                {(totp as any).qrSvg ? (
                  <div dangerouslySetInnerHTML={{ __html: (totp as any).qrSvg }} />
                ) : (
                  <div style={{ fontSize: 12, color: '#6B7280', padding: 12 }}>
                    QR no disponible en esta versión del SDK.<br/>Usa el secreto:<br/>
                    <code>{(totp as any).secret ?? '—'}</code>
                  </div>
                )}
              </div>
        ...
        <div style={{ marginTop: 16, fontSize: 12, color: '#6B7280' }}>
          Nota: el registro completo de actividad reciente (IPs, inicios de sesión, cambios críticos) requiere una tabla de auditoría propia. Aún no está implementado.
        </div>
