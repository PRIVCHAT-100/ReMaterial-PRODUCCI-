  ...
  try {
    ...
    if (mode === 'payment') {
      ...
      const platformFee = typeof applicationFeeAmountCents === 'number'
        ...
        : Math.floor(amount * (feeBps / 10000));

      params = {
        ...params,
        line_items: [{ price_data: { currency, unit_amount: amount, product_data: { name: (metadata as any)?.product_name || 'Compra en ReMaterial' } }, quantity: 1 }],
        payment_intent_data: { application_fee_amount: platformFee, transfer_data: { destination: sellerAccountId } },
      };
    } else {
      ...
      params = {
        ...
        subscription_data: { transfer_data: { destination: sellerAccountId }, application_fee_percent: bpsToPercent(feeBps) },
      };
    }
