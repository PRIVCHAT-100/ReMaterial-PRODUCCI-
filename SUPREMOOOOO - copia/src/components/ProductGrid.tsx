...
const ProductGrid = ({
  ...
  // 🆕 Estado para información de reservas unificado
  const [reservedProducts, setReservedProducts] = useState<Record<string, {
    reserved: boolean;
  ...
  // 🆕 Verificar reservas de productos
  useEffect(() => {
    const checkReservedProducts = async () => {
      const productIds = products.map(p => p.id);
      if (productIds.length === 0) return;

      try {
        // Buscar conversaciones para estos productos
        ...
        const productConversations: Record<string, string[]> = {};
        
        conversations.forEach(conv => {
          if (conv.product_id) {
            if (!productConversations[conv.product_id]) {
              productConversations[conv.product_id] = [];
            }
            productConversations[conv.product_id].push(conv.id);
          }
        });

        // Buscar ofertas reservadas para estas conversaciones
        const { data: offers, error: offersError } = await supabase
          .from('offers')
          .select('conversation_id, reserved, reserved_quantity, reserved_price')
        ...
        // Calcular reservas por producto
        const reservedData: Record<string, {
          reserved: boolean;
          reserved_quantity?: number;
          reserved_price?: number;
        }> = {};

        Object.keys(productConversations).forEach(productId => {
          const convIds = productConversations[productId];
          const productOffers = offers?.filter(offer => convIds.includes(offer.conversation_id)) || [];
          
          if (productOffers.length > 0) {
            // 🆕 SUMAR todas las cantidades reservadas
            const totalReserved = productOffers.reduce((sum, offer) => {
              return sum + (offer.reserved_quantity || 0);
            }, 0);

            // 🆕 Tomar el precio de la primera oferta
            const firstOfferPrice = productOffers[0]?.reserved_price;

            reservedData[productId] = {
              reserved: true,
              reserved_quantity: totalReserved,
              reserved_price: firstOfferPrice
            };
          } else {
            reservedData[productId] = { reserved: false };
          }
        });

        setReservedProducts(reservedData);
      } catch (error) {
        console.error('Error checking reserved products:', error);
      }
    };
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        if (conversations && conversations.length > 0) {
          ...
          const reservedData: Record<string, any> = {};
              ...
              <>
                <div className={viewMode === "grid" ? "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4" : "space-y-3"}>
                  {pageItems.map((product) => {
                    const reservedInfo = reservedProducts[product.id] || { reserved: false };
                    ...
                    return (
                      <div key={product.id} className="relative">
                        ...
                        <ProductCard
                          ...
                          unit={product.unit}
                          // 🆕 Pasar información de reserva para unificar visualización
