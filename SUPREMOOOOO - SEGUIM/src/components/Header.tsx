...
const Header = () => {
  ...
  // Detecta rutas de Empresas para aplicar espaciador visual (sin mostrar buscador)
  const isEmpresas = useMemo(() => {
    const p = location.pathname?.toLowerCase?.() ?? "";
    return p === "/empresas" || p.startsWith("/empresas/") || p.includes("empresa");
  }, [location.pathname]);
...
<form
  ...
  <div className="hidden md:block flex-1 max-w-xl mx-4" aria-hidden="true" />
) : null)}
